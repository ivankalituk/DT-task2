...
const CountryListPage: FC = () => {
  const { data: countries, isFetched: countriesFetched } = useGetRequest<
    SimpleCountry[]
  >({ fetchFunc: () => getAllCountries(), key: [], enabled: true });
    ...
    <>
      <div className="countryList_list">
        {countries &&
          countriesFetched &&
          countries.length > 0 &&
          countries.map((data: SimpleCountry, index: number) => (
            <Link to={`/countryInfo/${data.countryCode}`} key={index}>
              {data.name}
            </Link>
          ))}
      </div>
